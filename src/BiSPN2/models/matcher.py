...
class TripleMatcher(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        """
        bsz, num_generated_triples = outputs["pred_rel_logits"].shape[:2] # 3, 40
        ...
        if self.matcher == "avg":   # size([bsz * triple_query_num(120), triple_query_num(40)]) 
        ...
        cost = cost.view(bsz, num_generated_triples, -1).cpu()      # size([bsz(3), num_generated_triples(40), triple_queries_num(40)])
        num_gold_triples = [len(v["relation"]) for v in targets]    # e.g. [20, 20, 0]
...
class EntityMatcher(nn.Module):
    ...
    def __init__(self, loss_weight, matcher, boundary_softmax=False):
        super().__init__()
        self.cost_ent_type = 1.0
        self.cost_ent_span = 1.0
    ...
    def forward(self, outputs, targets):
        ...
        # 将前两个维度合并，然后在最后一个维度上应用softmax,  size([3*80, 39])
        pred_ent_type = outputs["ent_type_logits"].flatten(0, 1).softmax(-1)  # [bsz * num_generated_entities, num_ent_types]
        gold_ent_type = torch.cat([v["ent_type"] for v in targets]) # size([3*50])
        ...
        if self.boundary_softmax:
            ...
            pred_ent_start = outputs["p_left"].flatten(0, 1)  # size([3*80, 94])
            pred_ent_end = outputs["p_right"].flatten(0, 1)   # size([3*80, 94])

        gold_ent_start = torch.cat([v["ent_start_index"] for v in targets]) # size([3*50])
        ...
        cost = cost.view(bsz, num_generated_entities, -1).cpu()    # size([3, 80, 150])
        num_gold_entities = [len(v["ent_type"]) for v in targets]   # [50, 50, 50]
        indices = [linear_sum_assignment(c[i]) for i, c in enumerate(cost.split(num_gold_entities, -1))]  # size([3][2][50])
...
class TripleMatcher2(nn.Module):
    """This class computes an assignment between the targets and the predictions of the network
    For efficiency reasons, the targets don't include the no_object. Because of this, in general,
    there are more predictions than targets. In this case, we do a 1-to-1 matching of the best predictions,
    while the others are un-matched (and thus treated as non-objects).
    ...
    @torch.no_grad()
    def forward(self, outputs, targets, head_ent_scores, tail_ent_scores, compute_match_loss):
        """ Performs the matching
        ...
        if not compute_match_loss:
            ...
            cost = - self.cost_relation * pred_rel[:, gold_rel] - self.cost_head * (pred_head_start[:, gold_head_start] + pred_head_end[:, gold_head_end]) - self.cost_tail * (pred_tail_start[:, gold_tail_start] + pred_tail_end[:, gold_tail_end])

        else:
            pred_head_entID = head_ent_scores.flatten(0, 1).softmax(-1)  # [bsz * num_generated_triples, num_entities]
            pred_tail_entID = tail_ent_scores.flatten(0, 1).softmax(-1)  # [bsz * num_generated_triples, num_entities]

            gold_head_entID = torch.cat([v["head_entID"] for v in targets])
            gold_tail_entID = torch.cat([v["tail_entID"] for v in targets])

            cost = - self.cost_relation * pred_rel[:, gold_rel] - self.cost_head * pred_head_entID[:, gold_head_entID] - self.cost_tail * pred_tail_entID[:, gold_tail_entID]

        # cost = - self.cost_relation * pred_rel[:, gold_rel] - self.cost_head * (pred_head_start[:, gold_head_start] + pred_head_end[:, gold_head_end] + 
        # pred_head_entID[:, gold_head_entID]) - self.cost_tail * (pred_tail_start[:, gold_tail_start] + pred_tail_end[:, gold_tail_end] + pred_tail_entID[:, gold_tail_entID])
...
class TripleMatcher2_(nn.Module):
    """This class computes an assignment between the targets and the predictions of the network
    For efficiency reasons, the targets don't include the no_object. Because of this, in general,
    there are more predictions than targets. In this case, we do a 1-to-1 matching of the best predictions,
    while the others are un-matched (and thus treated as non-objects).
    ...
    @torch.no_grad()
    def forward(self, outputs, targets, head_ent_scores, tail_ent_scores):
        """ Performs the matching
        ...
        bsz, num_generated_triples = outputs["pred_rel_logits"].shape[:2]
        # We flatten to compute the cost matrices in a batch
        pred_rel = outputs["pred_rel_logits"].softmax(-1)  # [bsz, num_generated_triples, num_classes]
        gold_rel = [v["relation"] for v in targets]

        pred_head_entID = head_ent_scores.softmax(-1)  # [bsz, num_generated_triples, num_entities]
        pred_tail_entID = tail_ent_scores.softmax(-1)  # [bsz, num_generated_triples, num_entities]

        gold_head_entID = [v["head_entID"] for v in targets]
        gold_tail_entID = [v["tail_entID"] for v in targets]

        indices = []
        for pred_rel_, gold_rel_, pred_head_entID_, gold_head_entID_, pred_tail_entID_, gold_tail_entID_ in zip(
            pred_rel, gold_rel, pred_head_entID, gold_head_entID, pred_tail_entID, gold_tail_entID):

            cost = - self.cost_relation * pred_rel_[:, gold_rel_] - self.cost_head * pred_head_entID_[:, gold_head_entID_] - self.cost_tail * pred_tail_entID_[:, gold_tail_entID_]
            indice = linear_sum_assignment(cost.cpu())
