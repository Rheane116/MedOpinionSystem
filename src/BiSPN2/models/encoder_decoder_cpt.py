...
class EntityAwareBertConfig(CPTConfig):
    def __init__(self, num_generated_triples=None, 
...
class EntityAwareBertSelfAttention(nn.Module):
    ...
    def transpose_for_scores(self, x):
        new_x_shape = x.size()[:-1] + (self.num_attention_heads, self.attention_head_size)  # (3,94)+(16,64) = (3,94,16,64)
        x = x.view(*new_x_shape)
        return x.permute(0, 2, 1, 3)  # (3,16,94,64)
    # size:           [3, 94, 1024]
    def forward(self, token_hidden_states, entity_hidden_states, attention_mask, query_pos = None):
        context_size = token_hidden_states.size(1)    # context的token数目

        pos_aware_entity_hidden_states = entity_hidden_states
        if query_pos is not None:
            pos_aware_entity_hidden_states = (entity_hidden_states + query_pos)/2   # 取token编码和pos编码的均值
            # pos_aware_entity_hidden_states = entity_hidden_states + query_pos
        # query specific    token的query向量 size([3, 16, 94, 64])
        ...
        else:   # entity和relation的query向量 size([3, 16, 120, 64])
            e2w_query_layer = self.transpose_for_scores(self.query(pos_aware_entity_hidden_states))


        # key unified transformered    token的key向量 size([3, 16, 94, 64])
        ...
        else:    # entity和relation的value向量 size([3, 16, 120, 64])
            e2w_value_layer = self.transpose_for_scores(self.value(entity_hidden_states))

        w2w_attention_scores = torch.matmul(w2w_query_layer, w2w_key_layer.transpose(-1, -2)) # size([3, 16, 94, 94])
        w2e_attention_scores = torch.matmul(w2w_query_layer, e2w_key_layer.transpose(-1, -2)) # size([3, 16, 94, 120])
        e2w_attention_scores = torch.matmul(e2w_query_layer, w2w_key_layer.transpose(-1, -2)) # size([3, 16, 120, 94])
        # w2e_attention_scores = torch.zeros(e2w_attention_scores.size()).transpose(-1, -2).to(e2w_attention_scores.device) - 1e30
        e2e_attention_scores = torch.matmul(e2w_query_layer, e2w_key_layer.transpose(-1, -2)) # size([3, 16, 120, 120])

        word_attention_scores = torch.cat([w2w_attention_scores, w2e_attention_scores], dim=3) # size([3, 16, 94, 214])
        entity_attention_scores = torch.cat([e2w_attention_scores, e2e_attention_scores], dim=3) # size([3, 16, 120, 214])
        attention_scores = torch.cat([word_attention_scores, entity_attention_scores], dim=2) # size([3, 16, 214, 214])

        attention_scores = attention_scores / (self.attention_head_size**0.5)
        attention_scores = attention_scores + attention_mask    # 考虑注意力掩码    size([3, 16, 214, 214])

        attention_probs = F.softmax(attention_scores, dim=-1)   # 对最后一个维度进行softmax
        attention_probs = self.dropout(attention_probs)

        # value unified transformered
        value_layer = torch.cat([w2w_value_layer, e2w_value_layer], dim = -2)   # size([3, 16, 214, 64])
        context_layer = torch.matmul(attention_probs, value_layer)              # size([3, 16, 214, 64])

        context_layer = context_layer.permute(0, 2, 1, 3).contiguous()  # size([3, 214, 16, 64]), contiguous方法保证张量在内存中连续存储
        new_context_layer_shape = context_layer.size()[:-2] + (self.all_head_size,) # (3, 214, 1024)
        context_layer = context_layer.view(*new_context_layer_shape)     # size([3, 214, 1024])
        # 返回tokens和entity/relation各自的注意力向量
...
class EntityAwareBertSelfOutput(nn.Module):
    ...
    def forward(self, token_self_output, entity_self_output, token_hidden_states, entity_hidden_states):
        ...
        token_self_output = self.LayerNorm(token_self_output + token_hidden_states) # embeddings + output
...
class EntityAwareBertAttention(nn.Module):
    ...
    def forward(self, word_hidden_states, entity_hidden_states, attention_mask, query_pos = None):
        word_self_output, entity_self_output = self.self(word_hidden_states, entity_hidden_states, attention_mask, query_pos = query_pos) # tokens和entity/relation各自的注意力向量
        output = self.output(word_self_output, entity_self_output, word_hidden_states, entity_hidden_states) # size([3,214,1024])
...
class EntityAwareBertOutput(nn.Module):
    ...
    def forward(self, token_intermediate_output, entity_intermediate_output, word_attention_output, entity_attention_output):
        ...
        token_intermediate_output = self.LayerNorm(token_intermediate_output + word_attention_output) # 残差连接，层归一化
...
class EntityAwareBertEncoder(nn.Module):
    ...
    #size:            [3, 94, 1024]       [3, 120, 1024]        [3, 1, 214, 214]  [3, 120, 1024]
    def forward(self, word_hidden_states, entity_hidden_states, attention_mask, query_pos = None):
...
class SelfCrossAttention(nn.Module):
    ...
    def forward(self, h_token, h_entity, token_entity_attention_mask, query_pos = None):
        ...
        for i in range(self.num_selfcrosslayer):
            ...
            if self.use_token_level_encoder: # size: [3,seq_len,1024] [3, ent_query_num + rel_query_num*3, 1024]
                h_token, h_entity = self.crosslayers[i](h_token, h_entity, token_entity_attention_mask, query_pos = query_pos)

            if self.use_entity_attention: # size:([3,200,1024])
...
class EntityBoundaryPredictor(nn.Module):
    ...
    # size:             [3,94,1024]    [3,80,1024]         [3,94]
    def forward(self, token_embedding, entity_embedding, token_mask):
        # B x #ent x #token x hidden_size       # size([3,80,94,1024])
        ...
        entity_token_cls = self.boundary_predictor(torch.relu(entity_token_matrix))     # size([3, 80, 94, 1])
        token_mask = token_mask.unsqueeze(1).expand(-1, entity_token_cls.size(1), -1)   # size([3, 80, 94])
   
        if self.squeeze:
            entity_token_cls = entity_token_cls.squeeze(-1)   # size([3, 80, 94])
        else:
...
class HeadTailBoundaryPredictor(nn.Module):
    ...
    def forward(self, token_embedding, entity_embedding, token_mask):
        # B x #ent x #token x hidden_size   # size([3, 40, 94, 1024])
        entity_token_matrix = self.token_embedding_linear(token_embedding).unsqueeze(1) + self.entity_embedding_linear(entity_embedding).unsqueeze(2)
        entity_token_cls = self.boundary_predictor(torch.relu(entity_token_matrix))     # size([3, 40, 94, 1])
        token_mask = token_mask.unsqueeze(1).expand(-1, entity_token_cls.size(1), -1)   # size([3, 40, 94])

        if self.squeeze:
            entity_token_cls = entity_token_cls.squeeze(-1)     # size([3, 40, 94])
        else:
            token_mask = token_mask.unsqueeze(-1)
        entity_token_cls[~token_mask] = -10000
...
class EntityTypePredictor(nn.Module):
    ...
    # shape:        [3,80,1024] [3,94,1024] [3,80,94] [3,80,94] [3,94]
    def forward(self, h_entity, h_token, p_left, p_right, token_mask):
        ...
        attn_output = attn_output.transpose(0, 1)   # size([3,80,1024])
...
class EntityAwareBertModel(BertPreTrainedModel):
    ...
    # word_attention_mask:    token_attention_mask             size([3, 94])
    # entity_attention_mask:  entity_triple_attention_mask     size([3, 120]) / size([3, 200])
    def _compute_extended_attention_mask(self, word_attention_mask: torch.LongTensor, entity_attention_mask: torch.LongTensor, mask_ent2tok = None, mask_tok2ent = None, mask_ent2ent = None, mask_entself = None, seg_mask = None):
        ...
        if entity_attention_mask is not None:
            # attention_mask: size([3, 214]) / size([3, 294])
        ...
        # unsqueeze()方法： 插入一个维度为1的维度 
        # expand()方法： 除维度2外其余不变

        # extended_attention_mask: size([3, 1, 214, 214]) / size([3, 1, 294, 294])
        ...
        # 这行代码利用了PyTorch的高级索引功能。由于mask是一个布尔型张量，
            # entity_attention[mask]会选择所有在mask中为True的位置（即原单位矩阵的对角线位置，现在扩展到与entity_attention相同的形状）。然后，将这些位置的值设置为0。
        # 这里的目的是将entity_attention张量中对角线上的元素（对应于每个实体与自身的注意力）置零。
            # 这通常用于避免模型在自注意力机制中过度关注自身，从而鼓励模型关注其他实体。
        ...
        # 将 extended_attention_mask 的数据类型(dtype)转换为模型当前参数的数据类型。
        extended_attention_mask = extended_attention_mask.to(dtype=next(self.parameters()).dtype)
        # 将原始掩码中值为1的变为0， 值为0的变为-10000（大负数）
    ...
    def forward(self, token_input_ids, token_attention_mask, entity_ids, triple_ids, entity_triple_attention_mask=None, seg_encoding=None):
        # token_input_ids : size([3, 94])
        # entity_ids : size([3, 80])
        # triple_ids : size([3, 40])
        
        # word_embeddings : size([3, 94, 1024])
        word_embeddings = self.embeddings(token_input_ids, token_type_ids = seg_encoding)
        # entity_embeddings : size([3, 80, 1024])
        entity_embeddings = self.entity_embeddings(entity_ids)
        if self.RE:
            # triple_embeddings : size([3, 40, 1024])
            triple_embeddings = self.relation_embeddings(triple_ids)
            # entity_triple_embeddings : size([3, 120, 1024])
...
class DecoderLayer(nn.Module):
    ...
    def forward(
        self,
        hidden_states,              # h_relation,  size([3,40,1024])
        encoder_hidden_states,      # h_token_,    size([3,94,1024])
        encoder_attention_mask      # token_masks_float  size([3,94])
    ):
        self_attention_outputs = self.attention(hidden_states)  # Size: ([3, 40, 1024])
        attention_output = self_attention_outputs[0]
        outputs = self_attention_outputs[1:]  # add self attentions if we output attention weights

        encoder_batch_size, encoder_sequence_length, _ = encoder_hidden_states.size()
        encoder_hidden_shape = (encoder_batch_size, encoder_sequence_length)  # (3,94)
        if encoder_attention_mask.dim() == 3:
            encoder_extended_attention_mask = encoder_attention_mask[:, None, :, :]
        elif encoder_attention_mask.dim() == 2:
            encoder_extended_attention_mask = encoder_attention_mask[:, None, None, :]  # size([3,1,1,94])
        else:
            raise ValueError(
        ...
        cross_attention_outputs = self.crossattention( # cross_attention_outputs:  size([3, 40, 1024])
        ...
        attention_output = cross_attention_outputs[0] # size([3, 40, 1024])
        outputs = outputs + cross_attention_outputs[1:]  # add cross attentions if we output attention weights

        intermediate_output = self.intermediate(attention_output)
        layer_output = self.output(intermediate_output, attention_output)
        outputs = (layer_output,) + outputs
        return outputs      # shape: [3, 40, 1024]
...
class PIQN(PreTrainedModel):
    ...
    def __init__(self, model_type, 
                 ...
                 num_token_ent_rel_layer = 2, 
                 num_token_ent_layer = 2, 
                 num_token_rel_layer = 1, 
                 num_token_head_tail_layer = 2, 
        ...
        if use_char_lstm:
            ...
            self.char_lstm = nn.LSTM(input_size = char_size, 
    ...
    def forward(self, encodings: torch.tensor, 
        ...
        # size: [batch_size, num_generated_triples]
        ...
        # word_hidden_states, entity_hidden_states, intermediate    
        # size: [bsz, seq_len, 1024]   [bsz, ent_query_num + rel_query_num * 3, 1024]
        ...
        if self.RE:
            ...
            h_rel_head_tail = torch.cat([h_triple, h_head, h_tail], dim=1)  # size([3, ent_query_num + rel_query_num, 1024])
            h_rel_head_tail = self.LayerNorm1(h_rel_head_tail)
            h_ent_rel_head_tail = torch.cat([h_entity, h_rel_head_tail], dim=1)  # size([3, ent_query_num + rel_query_num * 3, 1024])
        ...
        if self.config.use_entity_pos and self._share_query_pos and self.use_token_level_encoder:
            ent_query_pos = self.model.pos_entity_embeddings(entity_ids)        # size([3, ent_query_num, 1024])
            if self.RE:
                rel_query_pos = self.model.pos_triple_embeddings(triple_ids)    # size([3, rel_query_num, 1024])
        if not self._share_query_pos and self.use_token_level_encoder:
            ent_query_pos = self.pos_entity_embeddings(entity_ids)
            if self.RE:
                rel_query_pos = self.pos_triple_embeddings(triple_ids)
        ...
        if self.RE:
            query_pos = torch.cat([ent_query_pos, rel_query_pos, rel_query_pos, rel_query_pos], dim=1)  # size([3,200,1024])
        else:
            query_pos = ent_query_pos
                    
        
        # 送入shared_decoder
        # size: [3,seq_len,2014]  [3,ent_query_num + rel_query_num * 3,1024]     [4]
        ...
        if self.RE: 
            # 将h_entity_rel_head_tail拆开
            # size: [3,80,1024]  [3,40,1024]
            ...
            for i, layer_module in enumerate(self.Rel_layers):
                layer_outputs = layer_module( # size: [3,40,1024]  [3,94,1024]  [3,94]
                    h_relation, h_token_, token_masks_float
                )
                h_relation = layer_outputs[0]   # size([3,40,1024])
                rel_intermediate.append(h_relation)

        projects = []
        if self.RE:
            # 取shared_decoder输出的query部分的前两个,split成四个部分后加入projects
        ...
        for h_dict in ent_rel_head_tail_intermediate[-2:]:
            # 取shared_decoder输出的token和query部分的最后两个，并将query部分split成四个
            _h_token, _h_ent_rel_head_tail = h_dict["h_token"], h_dict["h_query"] # size: [3,94,1024]  [3,200,1024]
            if self.RE: # size: [3,80,1024]  [3,40,1024]  [3,40,1024] [3,40,1024]
                _h_entity, _h_rel, _h_head, _h_tail = torch.split(_h_ent_rel_head_tail, [self.config.entity_queries_num, self.config.num_generated_triples, self.config.num_generated_triples, self.config.num_generated_triples], dim=1)
            else:
        ...
        #     h_head, h_tail = torch.split(_h_head_tail, [self.config.num_generated_triples, self.config.num_generated_triples], dim=1)
            ...
            # Entity Decoder和Entity Predictor的部分
            logit_left, p_left = self.left_boundary_classfier(_h_token, _h_entity, token_masks) # size([3,80,94])
            logit_right, p_right = self.right_boundary_classfier(_h_token, _h_entity, token_masks)
            entity_logits = self.entity_classifier(_h_entity, _h_token, p_left, p_right, token_masks)   # size([3,80,39])
            # entity_logits = self.entity_classifier(_h_entity)
            logit_part, p_part = self.ent_part_classifier(_h_token, _h_entity, token_masks) if self.training else (None, None)  # size([3,80,94,1])
            ent_have_rel_logits = self.whether_ent_have_ent(_h_entity) if self.training else None   # size([3,80,2])            
            ...
            if self.RE:
                head_start_logits, p_head_start = self.head_start_classifier(_h_token, _h_head, token_masks) # size([3,40,94])
                ...
                head_part_logits, p_head_part = self.head_part_classifier(_h_token, _h_head, token_masks) if self.training else (None, None) # size([3,40,94,1])
                tail_part_logits, p_tail_part = self.tail_part_classifier(_h_token, _h_tail, token_masks) if self.training else (None, None)
                head_type_logits = self.head_type_classifier(_h_head)   # size([3, 40, 38])
                tail_type_logits = self.tail_type_classifier(_h_tail)

                class_logits = self.rel_classifier(h_relation)  # size([3, 40, 33])
