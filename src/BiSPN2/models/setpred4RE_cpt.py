...
from models.encoder_decoder_cpt import EntityAwareBertConfig, SeqEncoder, MatchPredictor
...
from utils.functions import generate_triple_with_etype
from utils.data import padded_stack


class SetPred4RE_cpt(nn.Module):

    def __init__(self, args, num_classes, num_ent_types, RE=True):
        super(SetPred4RE_cpt, self).__init__()
        self.args = args
        self.RE = RE

        config = EntityAwareBertConfig.from_pretrained(args.bert_directory, hidden_dropout_prob=args.dropout, 
                                                       num_generated_triples=args.num_generated_triples,
                                                        entity_queries_num = args.entity_queries_num,

                                                        mask_ent2tok = args.mask_ent2tok,  
                                                        mask_tok2ent = args.mask_tok2ent, 
                                                        mask_ent2ent = args.mask_ent2ent, 
                                                        mask_entself = args.mask_entself, 
                                                        entity_aware_attention = args.entity_aware_attention, 
                                                        entity_aware_selfout = args.entity_aware_selfout, 
                                                        entity_aware_intermediate = args.entity_aware_intermediate, 
                                                        entity_aware_output = args.entity_aware_output, 
                                                        use_entity_pos = args.use_entity_pos, 
        ...
        self.re_criterion = PIQNLoss(num_classes, loss_weight=self.get_loss_weight(args), 
                                     na_coef=args.na_rel_coef, 
                                     losses=["entity", "relation", "head_tail_part", "head_tail_type"],
                                        matcher=args.matcher, 
                                        loss_type='RE', 
                                        boundary_softmax=True, 
                                        hybrid=args.hybrid)
    ...
    # [bsz,seq_len,1024] [2][bsz,ent_query_num,*]  [2][bsz,rel_query_num,*]  [2,4]...
        ...
        if targets is not None:
            ...
            # 实体预测损失  size([2][batchsize][2][repeat_gt_entities])
            ...
            if self.RE: # 关系预测损失
                ...
                # print(f'ner_loss: {ner_loss}')
                # print(f're_loss: {re_loss}')
    ...
    def default_predict(self, input_ids, attention_mask, seg_encoding, context2token_masks, token_masks, info, ent_type_alphabet, rel_type_alphabet, filtrate=True):
        with torch.no_grad():
            rel_output, ent_output = self.forward(input_ids, attention_mask, seg_encoding, context2token_masks, token_masks)
            # print('default_predict', 'forward')
            pred_triple, entities = None, None
            if self.args.entity_queries_num > 0:
                entity_idxes, entities = generate_entity(ent_output, info, self.args, self.num_ent_types, ent_type_alphabet, map_index=True, filtrate=filtrate)
            if self.RE:
                # pred_triple = generate_triple(rel_output, info, self.args, self.num_classes, map_index=True)
                pred_triple = generate_triple_with_etype(rel_output, info, self.args, self.num_classes, self.num_ent_types, rel_type_alphabet, map_index=True)
    ...
    def batchify(self, batch_list, is_test=False):
        ...
        #ori_pos = [ele[11] for ele in batch_list] if any(x in self.args.dataset_name for x in ['child', 'cancer', 'baidu']) else None
        ori_pos = [ele[11] for ele in batch_list]
        category = [ele[12] for ele in batch_list] if any(x in self.args.dataset_name for x in ['cancer', 'baidu']) else None
        ...
        if self.args.use_gpu:
            input_ids = input_ids.cuda(self.device_id)
            attention_mask = attention_mask.cuda(self.device_id)
            seg_encoding = seg_encoding.cuda(self.device_id)
            context2token_masks = context2token_masks.cuda(self.device_id) if context2token_masks is not None else None
            token_masks = token_masks.cuda(self.device_id)

            if not is_test:
                targets_ = [{k: torch.tensor(v, dtype=(torch.float if 'labels' in k else torch.long), requires_grad=False).cuda(self.device_id)
        ...
        info = {"seq_len": sent_lens, "sent_idx": sent_idx, "text": text, "subword_to_word": subword_to_word, "inst": inst, "ori_pos": ori_pos, "category": category}
